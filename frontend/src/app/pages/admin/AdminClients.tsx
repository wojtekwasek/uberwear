...
const AdminClients = ({ userData }: { userData: UserData }) => {
    ...
    const fetchClients = async (start: number, search: string) => {
        try {
            const clients = await getClients(userData.access, start, pageSize, search);
    ...
    useEffect(() => {
        fetchClients(startIndex, searchQuery);
